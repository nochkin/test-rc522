...
#define PIN_RST RPI_GPIO_P1_03

void setcs(uchar b){
  bcm2835_gpio_write(PIN_CS, b==true?HIGH:LOW);
}
void setrst(uchar b){
  bcm2835_gpio_write(PIN_RST, b==true?HIGH:LOW);
}
...
void AntennaOn(void){
	...
	if (!(temp & 0x09)) //WAS 0x03 ORIGINALLY
		Write_MFRC522(TxControlReg, temp | 0x09);
...
void MFRC522_Reset(void){
    ...
    delay(50);
    do{
      val = Read_MFRC522(CommandReg);
    }while(val & 0b00010000 > 0);
}
...
void MFRC522_Init(void){
   ...
   do{
    ...
    // TPRescalerHi are the four low bits in TModeReg. TPrescalerLo is TPrescalerReg.
    Write_MFRC522(TModeReg, 0x80);	//Tauto=1; timer starts automatically at the end of the transmission
    Write_MFRC522(TPrescalerReg, 0xA9); //TPreScaler = 0x0A9 = 169 => f_timer = 40kHz; 25uS
    Write_MFRC522(TReloadRegL, 0x03);   // Reload timer with 0x3E8 = 1000, ie 25ms before timeout.           
    Write_MFRC522(TReloadRegH, 0xE8);

    Write_MFRC522(TxASKReg, 0x40);		//100%ASK Force 100% ASK modulation
    Write_MFRC522(ModeReg, 0x3D);		//CRC initilizate value 0x6363	???

//    Write_MFRC522(0x11, 0x1F); //ONLY FOR TESTING PURPOSE - disables the RF check on transmission
}
...
uchar MFRC522_ToCard(uchar command, uchar *sendData, uchar sendLen, uchar *backData, uint *backLen){
    ...
    do{
    	switch(Read_MFRC522(Status2Reg)&0x07){
    		case 0: printf("Modem is idle\n"); break;
    		case 1: printf("Waiting for StartSend\n"); break;
    		case 2: printf("TxWait: no RF field present\n");
//			printf("Antenna state: %d \n",Read_MFRC522(TxControlReg));
//    			AntennaOn();
			printf("Antenna state: %d \n",Read_MFRC522(TxControlReg));
			break;
    		case 3: printf("Transmitting...\n"); break;
    		case 4: printf("RxWait: no RF field present\n"); break;
    		case 5: printf("Waiting for data...\n"); break;
    		case 6: printf("Receiving...\n"); break;
    	}
	//CommIrqReg[7..0]
     ...
     if (status == MI_OK){
         ...
         if (status == MI_OK && ((sak & 0x04) == 0x00)){        
             ...
             if ((sak & 0x20) == 0x20){
                 ...
                 //if (dumpInfo)
                 //    printf(" UID complete, PICC compliant with ISO/IEC 14443-4\n");
             ...
             if (status == MI_OK)
             {
                 ...
                 printf("ANTICOLL error: cascading level 2\n");
             }
         }
   }//Anticoll
   else{
